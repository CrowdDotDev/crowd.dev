...
async function includeMemberOrganizations(
  qx: QueryExecutor,
  memberIds: string[],
): Promise<Map<string, IMemberOrganization[]>> {
  const memberOrganizations = await fetchManyMemberOrgs(qx, memberIds)
  const orgIds = uniq(
    memberOrganizations.reduce((acc, mo) => {
      acc.push(...mo.organizations.map((o) => o.organizationId))
  ...
  const result = new Map<string, IMemberOrganization[]>()
...
async function includeIdentities(
  qx: QueryExecutor,
  memberIds: string[],
): Promise<Map<string, IMemberIdentity[]>> {
  const identities = await fetchManyMemberIdentities(qx, memberIds)
  return new Map(identities.map(({ memberId, identities }) => [memberId, identities]))
}

async function includeSegments(
  qx: QueryExecutor,
  memberIds: string[],
  onlySubProjects: boolean,
  ...
  const segmentsInfo = segmentIds.length ? await fetchManySegments(qx, segmentIds) : []
  const segmentsInfoMap = new Map(segmentsInfo.map((s) => [s.id, s]))

  const result = new Map<string, any[]>()

  for (const ms of memberSegments) {
    const segments = (ms.segments || [])
      .map((segment) => {
        const segmentInfo = segmentsInfoMap.get(segment.segmentId)

        // include only subprojects if flag is set
        if (onlySubProjects && segmentInfo?.type !== SegmentType.SUB_PROJECT) {
          return null
        }
      ...
      .filter(Boolean)
    result.set(ms.memberId, segments)
  }

  return result
}

async function includeMaintainers(
  qx: QueryExecutor,
  memberIds: string[],
): Promise<Map<string, unknown[]>> {
  const maintainerRoles = await findMaintainerRoles(qx, memberIds)
  const maintainerSegmentIds = uniq(maintainerRoles.map((m) => m.segmentId))
  const maintainerSegmentsInfo = maintainerSegmentIds.length
    ? await fetchManySegments(qx, maintainerSegmentIds)
    : []

  const maintainerSegmentsInfoMap = new Map(maintainerSegmentsInfo.map((s) => [s.id, s]))
  const groupedMaintainers = groupBy(maintainerRoles, (m) => m.memberId)
  const result = new Map<string, unknown[]>()

  for (const [memberId, roles] of groupedMaintainers.entries()) {
    result.set(
      memberId,
      roles.map((role) => {
        const segmentInfo = maintainerSegmentsInfoMap.get(role.segmentId)
        return {
          ...role,
          segmentName: segmentInfo?.name,
        }
...
async function includeLfxMemberships(
  qx: QueryExecutor,
  organizationIds: string[],
): Promise<Map<string, LfxMembership>> {
...
export async function queryMembersAdvancedV2(
  ...
  const [memberOrganizations, identities, segments, maintainers] = await Promise.all([
    include.memberOrganizations ? includeMemberOrganizations(qx, memberIds) : undefined,
    include.identities ? includeIdentities(qx, memberIds) : undefined,
    include.segments ? includeSegments(qx, memberIds, include.onlySubProjects) : undefined,
