...
class MergeActionRepository {
  ...
  async getUnprocessedLLMApprovedSuggestions(
    ...
    return this.connection.query(
      ...
      ${type === EntityType.ORGANIZATION ? 'and exists (select 1 from organizations o1 where o1.id = l."primaryId") and exists (select 1 from organizations o2 where o2.id = l."secondaryId") and not (exists (select 1 from "lfxMemberships" lm1 where lm1."organizationId" = l."primaryId") and exists (select 1 from "lfxMemberships" lm2 where lm2."organizationId" = l."secondaryId"))' : ''}
      limit $(batchSize)
    `,
      {
        type,
