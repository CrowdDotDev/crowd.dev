...
import { DATA_SINK_WORKER_QUEUE_SETTINGS, SqsClient, SqsQueueReceiver } from '@crowd/sqs'
import {
  DataSinkWorkerQueueMessageType,
  IQueueMessage,
  ProcessIntegrationResultQueueMessage,
} from '@crowd/types'
import IntegrationStreamService from '../service/dataSinkService'
...
export class WorkerQueueReceiver extends SqsQueueReceiver {
  ...
  override async processMessage(message: IQueueMessage): Promise<void> {
    try {
      ...
      const service = new IntegrationStreamService(new DbStore(this.log, this.dbConn), this.log)
