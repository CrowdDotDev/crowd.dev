...
export async function testMemberBotCharacteristicAnalysis(
  ...
  for (const memberId of memberIds) {
    ...
    const PROMPT = `Your task is to analyze the following json document and decide if this member is a bot or a real human.
                  ...
                  Protips:
                  - Do not classify as bot solely based on one keyword in an identity if bio or displayName strongly indicate a real human.
                  - If only identities are provided, rely on them but adjust confidence based on how strongly they resemble automation patterns.
