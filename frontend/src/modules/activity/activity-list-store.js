...
import activityListExporterFields from '@/modules/activity/activity-list-exporter-fields'
...
export default {
  ...
  getters: {
    ...
    bulkActive: (state) => state.bulkActive,
  ...
  actions: {
    ...
    async doExport({ commit, getters }) {
      try {
        if (
          !activityListExporterFields ||
          !activityListExporterFields.length
        ) {
          throw new Error(
            'activityListExporterFields is required'
          )
        }
