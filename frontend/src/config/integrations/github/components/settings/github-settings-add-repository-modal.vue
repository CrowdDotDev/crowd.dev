      ...
      <div class="pt-4">
        <div v-if="!search || loading" class="flex flex-col items-center pb-6">
          ...
          <p v-else class="font-semibold text-medium">
            Explore the entire GitHub database and add any repository
          </p>
        </div>
        <div v-else class="min-h-104">
          ...
          <!-- Repositories -->
          <div v-if="tab === 'repositories'" class="flex flex-col gap-5">
          ...
          <!-- Organizations -->
          <div v-else-if="tab === 'organizations'" class="flex flex-col gap-5">
...
const addOrganizations = (org: any) => {
  ...
  repositories.value = [...repositories.value, ...org.repositories];
};
...
const searchForResults = () => {
  loading.value = true;
  // TODO: implement search logic from backend
  setTimeout(() => {
    loading.value = false;
  }, 300);
};
