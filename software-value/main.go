...
func main() {
	...
	for _, repoDir := range repoDirs {
		...
		fmt.Printf("Directory: %s - Estimated Cost in Dollars: %.2f\n", repoDir, report.Cocomo.CostInDollars)
...
func saveBatch(ctx context.Context, db *InsightsDB, batch []SCCReport) {
	for _, report := range batch {
		if err := db.saveProjectCost(ctx, report.Repository, report.Cocomo.CostInDollars, report.Repository.URL); err != nil {
