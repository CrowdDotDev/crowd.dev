...
import { findMemberById, MemberField, pgpQx, fetchMemberIdentities } from '@crowd/data-access-layer'
...
setImmediate(async () => {
  ...
  const secondary = await findMemberById(qx, secondaryId, [MemberField.ID, MemberField.DISPLAY_NAME])
  ...
  if (mergeActions.length > 0) {
    log.error(
      ...
      'Member merge already in progress. Resolve the existing merge before retrying!c',
    )
