...
import { SegmentCreateData, SegmentCriteria, SegmentData, SegmentLevel, SegmentProjectGroupNestedData, SegmentProjectNestedData, SegmentRawData, SegmentStatus, SegmentUpdateChildrenPartialData, SegmentUpdateData } from '../../types/segmentTypes'
...
> {
     ...
     * @param name 
     */
    async updateChildrenBulk(id:string, data: SegmentUpdateChildrenPartialData): Promise<SegmentData>{

        const segment = await this.findById(id)

        if (SegmentRepository.isProjectGroup(segment)){
            // update projects
            await this.updateBulk((segment as SegmentProjectGroupNestedData).projects.map( (p) => p.id), {
            ...
            // update subprojects
            await this.updateBulk((segment as SegmentProjectNestedData).subprojects.map( (sp) => sp.id), {
                parentName: data.name,
                parentSlug: data.slug
            })
        }
    ...
    async updateBulk(ids: string[], data: SegmentUpdateData): Promise<string[]> {
        ...
        const updateFields = Object.keys(data).filter( (key) => data[key] && (['name', 'slug', 'parentSlug', 'grandparentSlug', 'status', 'parentName', 'sourceId', 'sourceParentId', 'grandparentName'].includes(key)))
    ...
    override async update(id: string, data: SegmentUpdateData): Promise<SegmentData> {
        ...
        // strip arbitrary fields
        const updateFields = Object.keys(data).filter( (key) => (['name', 'slug', 'parentSlug', 'grandparentSlug', 'status', 'parentName', 'sourceId', 'sourceParentId', 'customActivityTypes', 'activityChannels'].includes(key)))
