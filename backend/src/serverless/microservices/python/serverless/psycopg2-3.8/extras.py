    ...
    def parse(self, s, cur, _bsdec=_re.compile(r"\\(.)")):
        ...
        for m in self._re_hstore.finditer(s):
            if m is None or m.start() != start:
                raise psycopg2.InterfaceError("error parsing hstore pair at char %d" % start)
        ...
        if start < len(s):
            raise psycopg2.InterfaceError("error parsing hstore: unparsed data after char %d" % start)
...
class CompositeCaster(object):
    ...
    def __init__(self, name, oid, attrs, array_oid=None, schema=None):
        ...
        if array_oid:
            self.array_typecaster = _ext.new_array_type((array_oid,), "%sARRAY" % name, self.typecaster)
        else:
