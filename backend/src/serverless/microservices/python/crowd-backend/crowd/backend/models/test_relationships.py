...
def test_find_all_activities_for_member(api: "Repository"):
    result = api.find_in_table(Member, {"id": "cc38b881-9d6a-4690-b1a7-bd4636b1ce82"})
...
def test_find_member_of_activity(api: "Repository"):
    ...
    assert result.parentMember is not None
    assert type(result.parentMember) == Member


def test_find_members_with_activities_after_time(api: "Repository"):

    query = api.session.query(Member)
    result = query.filter(
        Member.activities.any(Activity.timestamp >= datetime.datetime.fromisoformat("2022-02-01 16:17:52"))
    ).all()
